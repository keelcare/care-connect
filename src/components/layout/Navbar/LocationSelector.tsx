...
export function LocationSelector({ className, onClick }: LocationSelectorProps) {
    ...
    return (
        <button
            onClick={onClick}
            className={cn(
                "flex items-center gap-2 px-2 py-1 md:px-4 md:py-2 rounded-full",
        ...
        >
            ...
            <span className="max-w-[100px] md:max-w-[150px] truncate">{address}</span>
            <ChevronDown className="hidden md:block w-3 h-3 text-dashboard-text-secondary opacity-50" />
        </button>
    );
}
