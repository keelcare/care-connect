...
export const generateRecurrencePattern = (frequency: string, selectedDays: string[]): string => {
...
export const parseRecurrencePattern = (pattern: string): { frequency: string; days: string[] } => {
    if (pattern === 'DAILY') {
...
export const formatRecurrencePattern = (pattern: string): string => {
    ...
    if (pattern.startsWith('MONTHLY_')) {
        const days = pattern.replace('MONTHLY_', '').split('_');
        if (days.length === 1) return `${getOrdinal(parseInt(days[0]))} of every month`;
