...
const trustItems = [
  {
    icon: <ShieldCheck className="w-10 h-10 text-primary-900" />,
  ...
  {
    icon: <Heart className="w-10 h-10 text-primary-900" />,
            ...
            <div className="space-y-4">
               {['Premium vetting process', '24/7 Support line', 'Identity protection', 'Secure payments'].map((item) => (
                 <div key={item} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-primary-900 flex items-center justify-center text-white">
                        <ShieldCheck size={14} />
                    </div>
                    <span className="font-bold text-primary-900">{item}</span>
                 </div>
               ))}
            </div>
          </div>
