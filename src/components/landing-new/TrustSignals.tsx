    ...
    <section className="py-24 px-6 bg-[#F9F7F2]">
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl lg:text-5xl font-bold text-[#1B3022] mb-8 leading-tight">
              Your safety is our <br />
              <span className="text-[#CC7A68]">absolute priority.</span>
            </h2>
            ...
            <div className="space-y-4">
               {['Premium vetting process', '24/7 Support line', 'Identity protection', 'Secure payments'].map((item) => (
                 <div key={item} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-[#1B3022] flex items-center justify-center text-white">
                        <ShieldCheck size={14} />
                    </div>
                    <span className="font-bold text-[#1B3022]">{item}</span>
                 </div>
               ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {trustItems.map((item, idx) => (
              <div key={idx} className="bg-white p-10 rounded-[40px] shadow-xl shadow-emerald-900/5 hover:transform hover:-translate-y-2 transition-transform duration-300">
                ...
                <h3 className="text-xl font-bold text-[#1B3022] mb-4">{item.title}</h3>
