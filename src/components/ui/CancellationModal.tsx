...
export const CancellationModal: React.FC<CancellationModalProps> = ({
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={title || `Cancel ${type === 'request' ? 'Request' : 'Booking'}`}>
            <div className="space-y-6">
                ...
                {type === 'booking' && showFeeWarning && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="font-semibold text-amber-800 mb-1">Cancellation Fee Applies</h4>
                                <p className="text-sm text-amber-700">
                                    Since this booking starts within 24 hours ({hoursUntilStart?.toFixed(1)} hours from now), 
                                    a cancellation fee may be charged to compensate the caregiver for their reserved time.
                                </p>
                                <div className="mt-2 flex items-center gap-2 text-amber-800">
                                    <IndianRupee className="w-4 h-4" />
                                    <span className="font-medium">Estimated fee: Up to 25% of booking cost</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-red-50 border border-red-100 rounded-xl p-4">
                    <p className="text-sm text-red-700">
                        <strong>Note:</strong> This action cannot be undone. The {type === 'request' ? 'caregiver' : 'other party'} will be notified of the cancellation.
                    </p>
                </div>
