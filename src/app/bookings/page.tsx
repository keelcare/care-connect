    ...
    if (booking.status === 'COMPLETED') {
      ...
      if (!isPaid) {
        ...
        buttons.push(
          <div
            key="paid"
            className="flex items-center gap-1 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl border border-emerald-100 font-medium text-sm mr-2"
          >
            <Check size={14} />
            Paid
          </div>
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-10 lg:p-12 space-y-8">
        ...
        {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {requests
                  ...
                  .map((request) => (
                    <div
                      key={request.id}
                      className="group block bg-white rounded-2xl border border-stone-100 shadow-xl shadow-stone-200/50 transition-all duration-200 overflow-hidden"
                    >
                      <div className="p-6">
                        ...
                        <div className="space-y-3 text-sm text-stone-600">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span>
                              {new Date(
                                request.start_time.includes('T')
                                  ? request.start_time
                                  : `1970-01-01T${request.start_time}`
                              ).toLocaleTimeString('en-US', {
                                hour: 'numeric',
                                minute: '2-digit',
                                hour12: true,
                              })}{' '}
                              ({request.duration_hours} hrs)
                            </span>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-stone-500">
          ...
          (() => {
            ...
            const hasItems =
              filteredBookings.length > 0 || assignedRequests.length > 0;
              ...
              <div className="space-y-4">
                ...
                {filteredBookings.map((booking) => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className="bg-white p-6 rounded-2xl border border-stone-100 shadow-xl shadow-stone-200/50 flex flex-col md:flex-row md:items-center gap-6 hover:shadow-2xl transition-shadow"
                    >
                      <div className="flex items-center gap-4 flex-1">
