...
export default function AdminBookingDetailsPage() {
    ...
    const handleCancelBooking = async () => {
        if (!booking) return;
        const reason = prompt('Please provide a reason for cancellation (Admin Action):');
        if (!reason) return;

        try {
            setActionLoading(true);
            const updated = await api.bookings.cancel(booking.id, { reason: `Admin Cancelled: ${reason}` });
            setBooking(updated);
        } catch (err) {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-3xl font-bold text-neutral-900 font-display">Booking Details (Admin)</h1>
            </div>
                        ...
                        {actionLoading ? (
                            ...
                            <>
                                {booking.status === 'IN_PROGRESS' && (
                                    <Button className="rounded-xl shadow-lg hover:shadow-xl transition-all bg-green-600 hover:bg-green-700" onClick={handleCompleteBooking}>
                                        <CheckCircle className="mr-2" size={18} />
                                        Mark as Completed
                                    </Button>
                                )}

                                {(booking.status === 'CONFIRMED' || booking.status === 'IN_PROGRESS' || booking.status === 'REQUESTED') && (
