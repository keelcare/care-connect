...
export default function DirectBookingPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast({ message: `Booking request sent to ${nanny.profiles?.first_name}!`, type: 'success' });
            router.push('/bookings');

        } catch (error: any) {
            console.error("Booking failed:", error);
            addToast({ message: error.message || 'Failed to create booking', type: 'error' });
        } finally {
            setSubmitting(false);
        }
    };
        ...
        return (
            <ParentLayout>
                <div className="flex flex-col items-center justify-center min-h-screen">
                    <h2 className="text-xl font-bold text-neutral-900">Nanny not found</h2>
