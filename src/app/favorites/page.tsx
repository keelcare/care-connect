                    ...
                    {favorites.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {favorites.map((favorite) => {
                                ...
                                const nanny = favorite.users_favorite_nannies_nanny_idTousers || favorite.nanny;
                                if (!nanny) return null;

                                // Handle both full_name and first_name/last_name formats
                                const fullName = nanny.profiles 
                                    ? (nanny.profiles.full_name || `${nanny.profiles.first_name || ''} ${nanny.profiles.last_name || ''}`.trim())
                                    : 'Caregiver';
                                const hourlyRate = nanny.nanny_details?.hourly_rate 
                                    ? parseFloat(nanny.nanny_details.hourly_rate) 
                                    : 0;
                                const experienceYears = nanny.nanny_details?.experience_years || 0;
                                const bio = nanny.profiles?.bio || nanny.nanny_details?.bio || 'Professional caregiver ready to help your family.';
                                const location = nanny.profiles?.address || 'Location not specified';
                                ...
                                return (
                                    <ProfileCard
                                        ...
                                        isVerified={true}
                                        onViewProfile={() => router.push(`/caregiver/${favorite.nanny_id}`)}
