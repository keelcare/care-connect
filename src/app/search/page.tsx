...
import {
  FilterSidebar,
  FilterState,
} from '@/components/features/FilterSidebar';
...
export default function SearchPage() {
  ...
  const activeServiceCount = Object.values(filters.services).filter(
    Boolean
  ).length;
      ...
      if (isNearby) {
        ...
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            try {
              const { latitude, longitude } = position.coords;
              const response = await api.location.nearbyNannies(
                latitude,
                longitude,
                10
              );
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((nanny) => {
        ...
        const skills =
          nanny.nanny_details?.skills?.join(' ').toLowerCase() || '';
    ...
    if (selectedServices.length > 0) {
      filtered = filtered.filter((nanny) => {
        const nannySkills =
          nanny.nanny_details?.skills?.map((s) => s.toLowerCase()) || [];
        return selectedServices.some((service) => {
          ...
          if (service === 'housekeeping')
            return nannySkills.some(
              (s) => s.includes('house') || s.includes('clean')
            );
          if (service === 'tutoring')
            return nannySkills.some(
              (s) =>
    ...
    <ParentLayout>
      <div className="h-screen flex flex-col overflow-hidden bg-neutral-50">
        ...
        <div className="flex-1 flex gap-6 overflow-hidden px-4 md:px-6 pb-4">
          {/* Sidebar */}
          <div
            className={`hidden lg:block flex-shrink-0 overflow-y-auto h-full pb-20 custom-scrollbar overscroll-contain transition-all duration-300 ${isDesktopFilterOpen ? 'w-80 opacity-100' : 'w-0 opacity-0'}`}
          >
            {isDesktopFilterOpen && (
              <div className="pt-4">
